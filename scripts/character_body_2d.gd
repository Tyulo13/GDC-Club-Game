...
@export var isHooked : bool = false
...
func _physics_process(delta: float) -> void:
	...
	if IFRAMES > 0:
		...
		IFRAMES = 0
	if not isHooked: # if the player isn't attached to the grappling hook, apply movement as usual
	...
	if Input.is_action_just_released("grappling_hook"):
		...
		if is_instance_valid(hookclone):
			if isHooked:
				isHooked = false
				velocity += hookclone.get_node("HookPoint").linear_velocity
